...
public class ScanFile {
	...
	String[] type = {ControlMain.getProperty("label_filetypenotsupported"), "PVA (Video/Audio PES)", "MPEG-1 PS/SS (Video/Audio PES)",
			"MPEG-2 PS/SS (Video/Audio PES)", "PES (Video/Audio/TTX)", "PES (MPEG Audio)", "PES (private stream 1)", "ES (AC-3 Audio)",
			"ES (MPEG Audio)", "ES (MPEG Video)", "ES (AC-3 Audio) (psb. SMPTE)", "DVB/MPEG2 TS", "ES (DTS Audio)", //DM19122003 081.6
																													// int07
			"ES (DTS Audio) (psb. SMPTE)", "ES (RIFF Audio)", //DM30122003 081.6 int10
			"ES (compressed RIFF Audio)", "ES (Subpicture 2-bit RLE)" //DM31012004 081.6 int13
	};
	...
	public String Date(String file) {
		return java.text.DateFormat.getDateInstance().format(new java.util.Date(new File(file).lastModified()))
				+ "  "
				+ java.text.DateFormat.getTimeInstance(java.text.DateFormat.LONG).format(new java.util.Date(new File(file).lastModified()));
	}
	...
	public int AC3Audio(byte[] check) { //DM19122003 081.6 int07 changed
		audiocheck : for (int a = 0; a < 10000; a++) {
	...
	public void loadMPG2(byte[] check, int b, boolean vdr, boolean mpg1) {
		...
		mpg2check : for (int a = b; a < 500000; a += jump) {
		...
		try {
			if (vid.size() > 0)
				...
				video = ControlMain.getProperty("label_novideofound");
		} catch (Exception e) {
			video = ControlMain.getProperty("label_errorwhilechecking");
		}
		...
		try {
			if (ac3.size() > 0) {
				...
				audio = ControlMain.getProperty("label_noaudiofound");
		} catch (Exception e) {
			audio = ControlMain.getProperty("label_errorwhilechecking");
		}
	...
	public void checkVid(byte[] check) {
		...
		mpvcheck : for (int a = 0; a < check.length - 630; a++) {
	...
	public void PMTcheck(byte[] check, int a) {
		...
		video = "";
		audio = "";
		text = ""; //DM10032004 081.6 int18 add
		pidlist.clear();

		tscheck : for (; a < check.length - 1000; a++) {
			...
			if (bytecheck.size() < 188) {
				a += 188;
				addpack : for (; a < check.length - 500; a++) {
					if (check[a] != 0x47 || check[a + 188] != 0x47 || check[a + 376] != 0x47)
						continue addpack;
					if (((0x1F & check[a + 1]) << 8 | (0xFF & check[a + 2])) != pmtpid || (0x40 & check[a + 1]) != 0
							|| (0x30 & check[a + 3]) != 0x10) {
						a += 187;
						continue addpack;
					}
			...
			pidsearch : for (int b = 8, r = 8; b < pmt_len - 4 && b < pmt.length - 6; b++) {
				r = b;
	...
	public int testFile(String infile, boolean more) { //DM04122003 081.6_int02 changed
		video = ControlMain.getProperty("label_novideofound");
		audio = ControlMain.getProperty("label_noaudiofound");
		text = ControlMain.getProperty("label_noteletextfound");
		addInfo = "";
		playtime = "";
		hasVideo = false;
		nullpacket = false;
		...
		try {
			...
			riffcheck : for (int a = 0; a < bs0; a++) //DM30122003 081.6 int10 add, compressed as AC3,MPEG is currently better detected as
													  // ES-not RIFF
			{
			...
			supcheck : for (int a = 0; a < bs0; a++) //DM31012004 081.6 int13 add
			{
			...
			mpegtscheck : for (int a = 0; a < bs4; a++) { //DM17012004 081.6 int11 changed
				if (check[a] != 0x47 || check[a + 188] != 0x47 || check[a + 376] != 0x47 || check[a + 564] != 0x47
						|| check[a + 752] != 0x47)
			...
			audiocheck : for (int a = 0; a < bs0; a++) {
				/* DTS stuff taken from the VideoLAN project. */
				/* Added by R One, 2003/12/18. */
				...
				if (Audio.DTS_parseHeader(check, a) > 0) {
					...
					if (false) { //DM30122003 081.6 int10 new
						if (more)
							audio = Audio.DTS_saveAnddisplayHeader();
						playtime = getAudioTime(size);
						return 12;
					...
					if (true && Audio.MPA_parseNextHeader(check, a + Audio.Size) < 0) //DM30122003 081.6 int10 changed
						continue audiocheck;
					if (more) {
						audio = Audio.MPA_saveAnddisplayHeader();
						playtime = getAudioTime(size);
